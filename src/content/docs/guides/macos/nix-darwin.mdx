...
lastUpdated: 2026-01-03 15:08:01
next: false
references:
...
import { nixDaemonServices, nixInstaller, removeNixGroupUsers, shellConfigBackups } from "@/lib/mdx-constants";
import { Code, Steps, TabItem, Tabs } from "@astrojs/starlight/components";
import configurationNix from "/src/files/config/nix/configuration.nix?raw";
import flakeNix from "/src/files/config/nix/flake.nix?raw";
import systemNix from "/src/files/config/nix/system.nix?raw";
...
2.  ### Add nix files
        ...
        <Code lang="nix" title="flake.nix" wrap preserveIndent code={flakeNix} />
        <Code lang="nix" title="configuration.nix" wrap preserveIndent code={configurationNix} />
        <Code lang="nix" title="system.nix" wrap preserveIndent code={systemNix} />

3.  ### Install nix

        <Code lang="sh" wrap preserveIndent code={nixInstaller} />

        - Insert `Y` to install Determinate Nix or `N` for NixOS.
...
4.  ### Install nix-darwin
        ...
        <Code lang="sh" wrap preserveIndent code={`nix run nix-darwin/master#darwin-rebuild -- switch --flake .#{{HOSTNAME_VAR}}`} />

5.  ### Build
        <Code lang="sh" wrap preserveIndent code={`darwin-rebuild switch --flake .#{{HOSTNAME_VAR}}`} />
