...
2. ### Copy ket to host (client)
   ...
   <Code lang="sh" wrap preserveIndent code={`scp ~/.ssh/id_rsa.pub {{USERNAME_VAR}}@{{IP_ADDRESS_VAR}}:~/.ssh/id_rsa.pub`} />
...
5. ### Add custom rule (optional)
   ...
   <Code lang="sh" wrap preserveIndent code={`sudo ufw allow from {{IP_ADDRESS_VAR}} port $PORT_NUMBER`} />
