...
lastUpdated: 2025-12-21 01:24:46
tableOfContents:
  maxHeadingLevel: 4
---

import { fail2banConfig, sudoersConfig, ufwSysctl, unattendedUpgradesConfig } from "@/lib/mdx-constants";
...
2. ### Modify config
   ...
   <Code
     lang="sh"
     frame="terminal"
     title="/etc/sudoers.d/010_{{USERNAME_VAR}}-nopasswd"
     wrap
     preserveIndent
     code={sudoersConfig}
   />
...
5.  ### Tailscale (optional)
    ...
    <br />
          ...
          1. #### Allow routed traffic
             ...
             <Code lang="sh" code={`sudo ufw route allow in on tailscale0`} />
          ...
          4. #### Enable packet forwarding
             ...
             <Code lang="sh" title="/etc/ufw/sysctl.conf" code={ufwSysctl} />

          </Steps>

6.  ### AdGuard Home (optional)

    Allow local network devices to send DNS queries to AdGuard Home.

    <Code lang="sh" wrap preserveIndent code={`sudo ufw allow from {{SUBNET_VAR}} to any port 53`} />
