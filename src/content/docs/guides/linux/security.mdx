...
2. <Code lang="sh" title="Copy key to server" code={`scp ~/.ssh/id_rsa.pub USER_NAME@DEVICE_IP:~/.ssh/id_rsa.pub`} />
...
5. **_Optional_** <Code lang="sh" title="Install ufw" code={`sudo ufw allow from $IP_ADDRESS port $PORT_NUMBER`} />
