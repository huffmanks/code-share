...
3. ### Modify config
   ...
   <Code lang="sh" frame="terminal" title="/etc/apt/apt.conf.d/02periodic" preserveIndent code={unattendedUpgradesConfig} />
...
2. ### Modify config
   ...
   <Code lang="sh" frame="terminal" title="/etc/sudoers.d/010_pi-nopasswd" preserveIndent code={sudoersConfig} />
...
2. ### Copy ket to host (client)
   ...
   <Code lang="sh" code={`scp ~/.ssh/id_rsa.pub {{{USERNAME_VAR}}}@{{{IP_ADDRESS_VAR}}}:~/.ssh/id_rsa.pub`} />

3. ### Add key to authorized keys (host)

   <Code lang="sh" code={`cat ~/.ssh/id_rsa.pub >> ~/.ssh/authorized_keys`} />
...
3. ### Modify config (optional)
   ...
   <Code lang="sh" frame="terminal" preserveIndent code={fail2banConfig} />
...
5. ### Add custom rule (optional)
   ...
   <Code lang="sh" code={`sudo ufw allow from {{{IP_ADDRESS_VAR}}} port $PORT_NUMBER`} />

   <Code lang="sh" code={allowOpenSSH} />
