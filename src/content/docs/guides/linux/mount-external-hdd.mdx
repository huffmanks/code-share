...
4.  ## Change ownership of mount point
    ...
    <Code lang="sh" code={`sudo chown -R $USER:$USER {{{EXTERNAL_HDD_PATH_VAR}}} && sudo chmod -R 755 {{{EXTERNAL_HDD_PATH_VAR}}}`} />
...
6.  ## Add mount entry to fstab
    ...
    <Code lang="sh" code={`UUID={{{EXTERNAL_HDD_UUID_VAR}}}  {{{EXTERNAL_HDD_PATH_VAR}}}       ext4            auto,nofail,noatime,rw,users,exec  0    0`} />
