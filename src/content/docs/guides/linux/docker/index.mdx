...
2. ### Install required keyrings
   ...
   <Code lang="sh" code={`sudo install -m 0755 -d /etc/apt/keyrings`} />
...
3. ### Add Docker GPG key
   ...
   <Code lang="sh" code={`sudo curl -fsSL https://download.docker.com/linux/debian/gpg -o /etc/apt/keyrings/docker.asc`} />
...
4. ### Change file permission
   ...
   <Code lang="sh" code={`sudo chmod a+r /etc/apt/keyrings/docker.asc`} />
...
2. ### Move docker data
   ...
   <Code lang="sh" code={`sudo mv /var/lib/docker {{{EXTERNAL_HDD_PATH_VAR}}}/docker-data`} />
...
5. ### Restart Docker
   ...
   <Code lang="sh" code={`sudo systemctl daemon-reload && sudo systemctl restart docker`} />
