...
pbpaste | ssh {{{USERNAME_VAR}}}@{{{IP_ADDRESS_VAR}}} 'cat > {{{DOCKER_PATH_VAR}}}/gitea/docker-compose.yml'
```
...
```sh
# *!* Copy content from next command before executing *!*
pbpaste | ssh {{{USERNAME_VAR}}}@{{{IP_ADDRESS_VAR}}} 'cat > {{{DOCKER_PATH_VAR}}}/gitea/.env'
```
...
{(<a href="http://localhost:3001" target="_blank" rel="noopener">http://localhost:3001</a>)}
