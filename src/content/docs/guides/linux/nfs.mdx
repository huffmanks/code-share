---
title: NFS
description: Setup guide for sharing files over a local network using NFS between Linux and Unix-like systems.
lastUpdated: 2026-03-22 18:22:48
---
...
2.  ### Add export
    ...
    <Code
      lang="sh"
      wrap
      code={`echo "{{NFS_SHARE_VAR}} {{CLIENT_IP_VAR}}(rw,sync,no_subtree_check)" | sudo tee -a /etc/exports`}
    />
...
3.  ### Manual mount
    ...
    <Code lang="sh" wrap code={`sudo mount {{SERVER_IP_VAR}}:{{NFS_SHARE_VAR}} {{NFS_MOUNT_VAR}}`} />
...
4.  ### Automount
    ...
    <Code
      lang="sh"
      wrap
      code={`{SERVER_IP_VAR}:{{NFS_SHARE_VAR}}	{{NFS_MOUNT_VAR}}	nfs	noauto,x-systemd.automount,_netdev,nofail,timeo=5,retrans=2	0	0`}
    />
...
5.  ### Unmount
    ...
    <Code lang="sh" code={`sudo umount -l {{NFS_MOUNT_VAR}}`} />
